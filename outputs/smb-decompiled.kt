...
fun moveObjectHorizontally(X: Int) {
    ...
    sprobjectXMoveforce[X] = (temp0 + memory[0x1].toInt()) and 0xFF
    //> lda #$00                    ;init A
    temp0 = 0x00
    //> rol                         ;rotate carry into d0
    temp0 = (temp0 shl 1) and 0xFE or if (temp0 + memory[0x1].toInt() > 0xFF) 1 else 0
    //> pha                         ;push onto stack
    push(temp0)
    ...
    sprobjectXPosition[X] = (temp0 + memory[0x0].toInt() + (if ((temp0 and 0x01) != 0) 1 else 0)) and 0xFF
    ...
    sprobjectPageloc[X] = (temp0 + memory[0x2].toInt() + (if (temp0 + memory[0x0].toInt() + (if ((temp0 and 0x01) != 0) 1 else 0) > 0xFF) 1 else 0)) and 0xFF
    //> pla
    temp0 = pull()
        ...
        if (temp0 == 0) {
            ...
            while (temp0 != loopCmdPageNumber[temp3]) {
                do {
                    ...
                    //> bne FindLoop
                } while (temp0 != loopCmdWorldNumber[temp3])
                do {
...
fun initNormalEnemy(X: Int) {
    ...
    // Fall-through tail call to initHorizFlySwimEnemy
    initHorizFlySwimEnemy(X)
}
...
fun runOffscrBitsSubs(A: Int, X: Int): Int {
    ...
    while (!flagN) {
        ...
        temp2 = yOffscreenBitsData[temp1]
        if (yOffscreenBitsData[temp1] == 0) {
            do {
                //> YOfsLoop: lda HighPosUnitData,y        ;load coordinate for edge of vertical unit
                temp2 = highPosUnitData[temp0]
                //> sec
                //> sbc SprObject_Y_Position,x   ;subtract from vertical coordinate of object
                //> sta $07                      ;store here
                memory[0x7] = ((temp2 - sprobjectYPosition[temp1]) and 0xFF).toUByte()
