...
fun AssemblyFunction.toKotlinFunction(
    ...
    // This happens when the last block of the function falls through to another function's entry
    val functionBlocks = run {
        val blocks = mutableSetOf<AssemblyBlock>()
        val toVisit = mutableListOf(this.startingBlock)
        while (toVisit.isNotEmpty()) {
            val b = toVisit.removeAt(0)
            if (!blocks.add(b)) continue
            if (b.function == this) {
                b.fallThroughExit?.let { if (it.function == this) toVisit.add(it) }
                b.branchExit?.let { if (it.function == this) toVisit.add(it) }
            }
        }
        blocks
    }
    ...
    for (block in functionBlocks) {
        ...
        if (fallThrough != null && fallThrough.function != this && fallThrough.function != null) {
            ...
            if (!bodyEndsWithReturn) {
                ...
                finalBody.add(KComment("Fall-through to $targetName", commentTypeIndicator = " "))
    ...
    return when {
        ...
        target is KMemberAccess && target.receiver is KVar &&
            ...
            if (value.isKnownByteSized()) {
                ...
                KMemberAccess(value, KCall("toUByte"), isIndexed = false)
            } else {
...
fun AssemblyAddressing?.toKotlinExpr(ctx: CodeGenContext): KotlinExpr {
    ...
    return when (this) {
        ...
        is AssemblyAddressing.ConstantReference -> {
            // Constants are uppercased in the constants file
            KVar(this.name.uppercase())
        }
        ...
        is AssemblyAddressing.IndirectX -> {
            ...
            val baseExpr: KotlinExpr = if (this.label.startsWith("$")) {
                ...
                // Named labels are uppercased to match constants file
                val baseLabel = this.label.uppercase()
                if (this.offset != 0) {
