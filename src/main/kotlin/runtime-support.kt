 ...
 * operMode = 1u  // Writes to memory[0x0770]
 * val x = operMode.toInt()  // Reads from memory[0x0770]
 * ```
 */
class MemoryByte(private val address: Int) {
    operator fun getValue(thisRef: Any?, property: kotlin.reflect.KProperty<*>): UByte {
        return memory[address and 0xFFFF]
    }

    operator fun setValue(thisRef: Any?, property: kotlin.reflect.KProperty<*>, value: UByte) {
...
class MemoryByteArray(private val baseAddress: Int) {
    operator fun get(index: Int): UByte {
        return memory[(baseAddress + index) and 0xFFFF]
    }

    operator fun set(index: Int, value: UByte) {
        memory[(baseAddress + index) and 0xFFFF] = value
    }
}
